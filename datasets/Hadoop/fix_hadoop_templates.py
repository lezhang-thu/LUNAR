...
TEMPLATE_FIXES = [
    {
        "id": 1,
        "name": "内存和核心格式修复",
        "find_exact": "<memory:<*>, vCores:<*>",  # 查找（没有右尖括号）
        "replace_with": "<memory:<*>, vCores:<*>>",  # 替换为（添加右尖括号）
...
def generate_report(original_data, modified_data, columns, template_col_idx, all_changes):
    ...
    timestamp = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    txt_report_path = os.path.join(REPORT_DIR, f"template_fixes_report_{timestamp}.txt")
...
def main():
    print_header("Hadoop 日志模板修复工具 - 精确版", "=")

    print_info(f"执行时间: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC")
